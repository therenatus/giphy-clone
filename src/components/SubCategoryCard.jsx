    ...
    return (
        <div className='w-full grid grid-cols-3 gap-8 mt-7'>
            {categories[category]?.subcategories.map((item, i) => (
                <div key={i} className=' relative w-full h-[100px] rounded-lg flex flex-col justify-center cursor-pointer hover:scale-105 transition duration-300 ease-in-out' style={{backgroundColor: `#${Math.random().toString(16).substr(-6)}`}}>
